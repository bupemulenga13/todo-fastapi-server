...
def get_labour_and_delivery_search_list(engine: Engine, start_date: str, end_date: str, patient_id: str) -> Optional[List[dict]]:
    """
    Returns a list of all labour and delivery data that match search criteria.
    ...
    select distinct
        ...
        when (A.PhoneNumber is not null and A.PhoneNumber = '' and A.MobilePhoneNumber is not null and A.MobilePhoneNumber <> '') then A.MobilePhoneNumber 
            ...
            from
                ...
                AND AND R.PatientID LIKE '%{patient_id}%'
                --right join Facility as F on F.HMISCode = DAI.overview_interaction_location
                --join District as D on D.DistrictSeq = F.DistrictId

            --Trevor: Commented out Login. Its unnecessary. Besides Login data is during FxD merge
            --join Login as L on L.LoginID = (select substring(DAI.overview_interaction_provider,0,charindex('|',DAI.overview_interaction_provider)))
            --where (A.AddressType = 'Current')
    """)
