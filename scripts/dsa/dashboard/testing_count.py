...
def get_testing_count(engine: Engine, start_date: str, end_date: str) -> Optional[List[dict]]:
    """
    Returns a count of testing date for the given date range.
  ...
  -- Update the Index entry point from Paed CBS second, only where "EntryPoint" values are null
    ...
    UPDATE c
    SET c.Phone = address.PhoneNumber + ' ' + address.MobilePhoneNumber,
        c.IndexAddress = address.StreetName + ' ' + address.CommunityName + ' ' + address.DirectionsToAddressFromClinic
    FROM #staging c
    ...
    select Nupin,ARTNumber,VisitDate,IndexName,Phone,IndexAddress,Age,Sex,EntryPoint,HIVTestDate,HIVResult,DateEnrolled,DateInitiated
    from #staging order by Nupin
    -- WHERE HIVTestDate BETWEEN @StartDate AND @EndDate       
""")
