...
def get_appointments_abnormal_vitals_list(engine: Engine, start_date: str, end_date: str) -> Optional[List[dict]]:
    """
    Returns a list of all appointments for clients with abnormal vitals.
    ...
    -- ============================================================================================
        ...
        select distinct
                ...
                ,CONVERT(varchar,vl1.InteractionDate, 103) as VLResultDate
                ,case
                    WHEN DATEDIFF(d, vl.DueDate, GETDATE()) > 0 THEN 'YES'
                    WHEN DATEDIFF(d, vl.DueDate, GETDATE()) < 1 THEN 'NO'
                end as DueForVL
                ,CASE
                    WHEN DATEDIFF(d, vl.DueDate, GETDATE()) > 0 THEN DATEDIFF(d,  GETDATE(), vl.DueDate) 
                    WHEN DATEDIFF(d, vl.DueDate, GETDATE()) < 1 THEN DATEDIFF(d,   GETDATE(), vl.DueDate) 
                END AS NextVLDay
                ,vt.Temperature
                ,CASE 
                WHEN vt.Temperature < 35 THEN 'Low Temp'
                WHEN vt.Temperature > 37.4 THEN 'High Temp'
                WHEN vt.Temperature BETWEEN 35 AND 37.4 THEN 'Normal Temp'
                END AS TempStatus
                ,vt.PulseRate as HeartRate
                ,CASE 
                WHEN vt.PulseRate < 60 THEN 'Low Heart Rate'
                WHEN vt.PulseRate > 100 THEN 'High Heart Rate'
                WHEN vt.PulseRate BETWEEN 60 and 100 THEN 'Normal Heart Rate'
                END AS HeartRateStatus
                ,vt.SystolicPressure
                ,CASE
                    WHEN vt.SystolicPressure > 140 THEN 'High Systolic Pressure'
                    WHEN vt.SystolicPressure < 90 THEN 'Low Systolic Pressure'
                    WHEN vt.SystolicPressure BETWEEN 90 AND 140 THEN 'Normal Systolic Pressure'
                END AS SystolicPressureStatus
                ,vt.RespiratoryRate
                ,CASE 
                    WHEN vt.RespiratoryRate > 20 THEN 'Hight Resp. Rate'
                    WHEN vt.RespiratoryRate < 12 THEN 'Low Resp. Rate'
                    WHEN vt.RespiratoryRate BETWEEN 12 and 20 then 'Normal Resp. Rate'
                END AS RespiratoryRateStatus
                ,CONVERT(VARCHAR,vt.InteractionTime, 103) AS InteractionTime
                ,dg.GenericName
        from #allDue al
        ...
        LEFT JOIN crtMedicationsDispensed dsp on al.PatientId = dsp.PatientId 
        AND dsp.Deprecated = 0 
        and dsp.ArvDrugDispensed = 1 
        and dsp.VisitDate = (SELECT MAX(VisitDate) FROM crtMedicationsDispensed WHERE al.PatientId = PatientId)
        ...
        LEFT JOIN ClientViralLoadTestResult vl1 on vl.PatientId = vl1.PatientId and vl.LastViralLoadResult = vl1.LabTestValue
        INNER JOIN (SELECT MAX(InteractionDate) as VLResultDate, PatientId FROM ClientViralLoadTestResult group by PatientId) vl2 on vl1.PatientId = vl2.PatientId and vl1.InteractionDate = vl2.VLResultDate
        ...
        LEFT JOIN crtVitals vt on al.PatientId = vt.PatientId
        INNER JOIN (
            SELECT MAX(InteractionTime) InteractionTime
            ,PatientId
            FROM crtVitals
            GROUP BY
            PatientId
        ) vt1 on vt.PatientId = vt1.PatientId AND vt.InteractionTime = vt1.InteractionTime
        LEFT JOIN InteractionOverviewNg itr on vt.InteractionID = itr.InteractionIdInt AND vt.InteractionTime = itr.InteractionTime AND vt.PatientId = itr.PatientIdInt
        LEFT JOIN Login lg on  CAST(itr.InteractionUser as VARCHAR(10)) = CAST(lg.LoginID as Varchar(10))
        WHERE vt.Temperature NOT BETWEEN 35 and 37.4
        
        AND al.PatientId IS NOT NULL
        OR vt.PulseRate NOT BETWEEN 60 and 100
        OR vt.SystolicPressure NOT BETWEEN 90 and 140
        OR vt.RespiratoryRate NOT BETWEEN 12 and 20

        -- WHERE vt.InteractionTime = (
        --     SELECT MAX(InteractionTime) FROM crtVitals 
        --     where vt.PatientId = PatientId
