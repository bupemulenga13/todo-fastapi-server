...
def get_appointments_tpt_list(engine: Engine, start_date: str, end_date: str) -> Optional[List[dict]]:
    """
    Returns a list of all appointments for clients on TB preventative treatment(tpt).
    ...
    sql = text(
    f"""
        IF(OBJECT_ID('tempdb..#TempTBTable')) IS NOT NULL
        DROP TABLE #TempTBTable;
        ...
        cs.[PatientId]
            ...
            ,[Sex]
            ,[RegistrationDate]
            ,[OldestHivPosTestDate]
            ,[ArtStartDate]
            ,[ArtStartLocation]
            ,[ArtStartEditLocation]
            ,[AgeOnArtStart]
            ,[IhapDate]
            ,[CTXStartDate]
            ,[CareStartDate]
            ...
            ,
            case
                WHEN MedBrandId = 473 or MedBrandId = 624 then 1
            else 0 end as TPTDispensed
        INTO #TempTBTable
        FROM [cdc_fdb_db].[dbo].[ClientHivSummary] cs
        inner join crtMedicationOrders ord on cs.PatientId = ord.PatientId
        inner join crctMedicationOrderDetail dt on dt.InteractionID = ord.InteractionID
        inner join crtMedicationsDispensed disp on ord.InteractionID = disp.InteractionID
        WHERE OldestHivPosTestDate is not null
        or ArtStartDate is not null
        or IhapDate is not NULL
        or CareStartDate is not null
        and cs.PatientDied = 0
            SET NOCOUNT ON
        ...
        -- ============================================================================================
            ...
            intr.InteractionIdInt as id,
                    ...
                    END AS NextVLDay
                    ,case
                    when tbl.TPTDispensed = 1 THEN 'Has TPT Interaction'
                else 'No TPT Interaction'
                end as TPTInteraction
                ,CASE 
                WHEN DATEDIFF(YYYY, tbl.VisitDate, GETDATE())  BETWEEN 0 and 4 and tbl.TPTDispensed = 1 then DATEDIFF(YYYY, tbl.VisitDate, GETDATE())
                WHEN DATEDIFF(YYYY, tbl.VisitDate, GETDATE())  BETWEEN 0 and 4 and tbl.TPTDispensed = 0 THEN NULL
                WHEN DATEDIFF(YYYY,tbl.VisitDate, GETDATE()) > 4 AND tbl.TPTDispensed = 1 THEN NULL
                ELSE NULL
                END as TPTDispensedInYears
                ,dg.GenericName as Regimen
                --    ,dsp.InteractionID
            from #allDue al
            ...
            left join #TempTBTable tbl on al.PatientId = tbl.PatientId
            LEFT JOIN InteractionOverviewNg intr on vl1.InteractionId = intr.InteractionIdInt AND intr.Deprecated = 0
            LEFT JOIN Login lg on  CAST(intr.InteractionUser as VARCHAR(10)) = CAST(lg.LoginID as Varchar(10))
            WHERE al.PatientId IS NOT NULL
            and intr.InteractionTime = (
                SELECT MAX(InteractionTime) from InteractionOverviewNg
                WHERE intr.InteractionIdInt = InteractionIdInt
            )
    """)
