...
def get_tx_current_search_list(engine: Engine, start_date: str, end_date: str, art_number: str) -> Optional[List[dict]]:
    """
    Returns a list of TX Curr indicator data that match search criteria.
...
--SET @EndDate = '2020-12-15 00:00:00'
...
SET @FacilityId = (select Value from Setting where Name = 'FacilityHmisCode')
   ...
   WHERE (
    ...
    -- WHERE NOT (crr.ArtStartDate > @EndDate or crr.ArtStartDate < @StartDate)
    WHERE reg.ArtNumber LIKE '{art_number}'
    AND NOT (crr.ArtStartDate > @EndDate)       
    """)
