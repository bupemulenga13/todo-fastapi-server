...
def get_tx_new_search_list(engine: Engine, start_date: str, end_date: str, art_number: str) -> Optional[List[dict]]:
    """
    Returns a list of TX New indicator data that match search criteria.
    ...
    SET @StartDate ='2020-01-01' --DATEADD(dd, DATEDIFF(dd, 0, @StartDate), 0)
    SET @EndDate = '2020-01-10'--DATEADD(dd, DATEDIFF(dd, 0, @EndDate), 0)
    SET NOCOUNT ON
  ...
  IF (object_id('tempdb..#TX_New_ClientsOfInterest')) IS NOT NULL
    ...
    select DISTINCT
		...
		vl.LabTestValue,
		vl.InteractionDate as VLResultDate,
		dt.NextVisitDate,
		sm.PriorClinicalVisitDate
		--wh.WhoStageToday,
		--wh.VisitDate
		...
		AND reg.ArtNumber LIKE '{art_number}'
