...
def get_extendTx_curr(engine: Engine, start_date: str, end_date: str) -> Optional[List[dict]]:
	"""
  ...
  DECLARE @LTFUThreshold INTEGER
    ...
    WHERE (
        ...
        reg.AgeAtDateOfEnrollment as ageAtEnrollment,
        vl.LabTestValue AS latestVlResult,
        CONVERT(varchar, vl.InteractionDate, 103) as vlResultDate,
        convert(varchar,sm.PriorClinicalVisitDate, 103) as lastClinicalVisit,
		pvist.numberofVisits pharmPicksYTD,
		cVisits.numberOfVisits as clinicalVisitsYTD,
		vlVisits.viralLoadVisits as vlVisitsYTD
	...
	INNER JOIN (SELECT
	COUNT(*) as viralLoadVisits,
	PatientId
	FROM ClientViralLoadTestResult
	-- BREAKPOINT
	GROUP BY PatientId) vlVisits ON vlVisits.PatientId = reg.patientId_int
	INNER JOIN (SELECT 
	COUNT(*) numberOfVisits, 
	PatientId
	FROM ClientHivSummary c
	WHERE c.LastClinicalVisit BETWEEN DATEADD(Year,-1, @EndDate) and  @EndDate 
	GROUP BY PatientId
	) cVisits on cVisits.PatientId = reg.patientId_int
	...
	count(*) numberofVisits,
	PatientId 
	from crtMedicationsDispensed c 
	where c.ArvDrugDispensed = 1 
	and c.Deprecated = 0 
	AND c.VisitDate between  DATEAdd(Year,-1, @EndDate) and  @EndDate 
	group by PatientId ) as pvist on reg.patientId_int = pvist.PatientId 
...
--	and st.PatientStoppedART = 0 and st.PatientTransferOut = 0 and st.PatientDied = 0
