...
def get_tx_new(engine: Engine, start_date: str, end_date: str) -> Optional[List[dict]]:
	"""
    Returns a list of tx new data
  ...
  IF (object_id('tempdb..#TX_New_ClientsOfInterest')) IS NOT NULL
    ...
    select reg.FirstName, reg.SurName, txNew.ArtStartDate,reg.AgeAtDateOfEnrollment,reg.ArtNumber from #TX_New_ClientsOfInterest txNew join crtRegistrationInteraction reg on reg.patientId_int = txNew.PatientId
