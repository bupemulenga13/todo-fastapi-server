...
def get_pharm_pick(engine: Engine, start_date: str, end_date: str) -> Optional[List[dict]]:
	"""
    Returns a list of pharm_pick data
	...
	select 
	tmp.FirstName + ' ' + tmp.SurName as ClientName
	,tmp.PhoneNumber
	,tmp.PObox + ', ' + tmp.HousePlotNumber + ', ' + tmp.StreetName as Address
	,tmp.ArtID as ArtNumber
	,r.PatientId as NUPN
	,tmp.LastPharmacyVisit  as LastPharmacyVisit
	,tmp.NextPharmacyVisit as NextPharmacyVisit
	,tmp.DaysLatePharmacy as DaysLate
	,intr.InteractionTime as LastInteractionDate
	,DATEDIFF(DAY, tmp.NextPharmacyVisit, intr.InteractionTime) as DifferenceDates
	,mp.MappedGuid
	,mp.OwningGuid
	,r.patientId_int as PatientId_Int
	...
	left join crtRegistrationInteraction r on r.patientId_int = tmp.PatientId
	LEFT JOIN GuidMap mp on r.patientId_int = mp.NaturalNumber
		--added this to remove patients that died/inactivated or transfered out using patient status
	left join crtPatientStatus stat on tmp.PatientId = stat.PatientId
	left join InteractionOverviewNg intr on tmp.PatientId = intr.PatientIdInt
	...
	where tmp.SurName is not null
		...
		and Category = 1 
		and intr.InteractionTime = (Select Max(InteractionTime) FROM InteractionOverviewNg WHERE  PatientIdInt = intr.PatientIdInt)
		--AND intr.InteractionTime > tmp.NextPharmacyVisit
