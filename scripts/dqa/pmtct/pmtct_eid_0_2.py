...
def get_pmtc_eid_0_2(engine: Engine, start_date: str, end_date: str) -> Optional[List[dict]]:
	"""
    Returns a list of pmtc_eid_0_2 data
	...
	f"""
		...
		select Nupin,
				#staging.ARTNumber,
				CONVERT(varchar, #staging.VisitDate, 103) as VisitDate,
				IndexName,
				...
				EntryPoint
		from #staging 
		LEFT JOIN crtRegistrationInteraction rg on #staging.PatientId = rg.patientId_int
		WHERE #staging.EntryPoint IS NOT NULL
		AND DATEDIFF(MONTH, rg.DateOfBirth, @EndDate) BETWEEN -1 and 3 
		order by Nupin 
