...
@router.post('/morbidity_list', response_model=query.GetList)
async def read_morbidity(req: Request, db: Session = Depends(get_db)):
    """
    Retrieve list of Morbidity.
    """
    req_body = await req.json()
    req_start_date = req_body.get('startDate')
    req_end_date = req_body.get('endDate')

    items = get_morbidity(db,req_start_date, req_end_date)
